...
An enormously useful feature of random draws, such as those produced by Bayesian analyses and stochastic simulations, is that they can be used to make inferences about quantities derived from the draws. The procedure is:
...
If we have a set of draws of age-specific mortality rates, for instance, and we want to make inferences about life expectancy (a deterministic function of mortality rates), then we proceed as follows:
...
Package **rvec** provides tools for working with random draws. The draws are held in a structure called an rvec, which can be manipulated using (mostly) standard base R and tidyverse code. **rvec** also contains functions for summarizing across random draws.
...
Packages [rv](https://CRAN.R-project.org/package=rv ) and [posterior](https://CRAN.R-project.org/package=posterior) contain similar data structures to rvecs, and package **rvec** borrows ideas from both of them. The advantages of rvecs is that they behave more like standard R vectors, which makes Step 1, the manipulation of draws, easier. 
